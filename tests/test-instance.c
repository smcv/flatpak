...
test_gc (void)
{
  ...
  g_autofree char *alive_app_xrd = NULL;
  ...
  g_autofree char *dead_app_xrd = NULL;
  ...
  alive_app_xrd = g_build_filename (alive_app_dir, "xdg-run", NULL);
  g_assert_no_errno (g_mkdir_with_parents (alive_app_xrd, 0700));
  populate_with_files (alive_app_xrd);
  alive_app_lock = g_build_filename (alive_app_xrd, ".ref", NULL);
  ...
  hold_lock_argv[2] = alive_app_lock;
  hold_lock_argv[4] = alive_instance_lock;
  ...
  dead_app_xrd = g_build_filename (dead_app_dir, "xdg-run", NULL);
  g_assert_no_errno (g_mkdir_with_parents (dead_app_xrd, 0700));
  populate_with_files (dead_app_xrd);
  dead_app_lock = g_build_filename (dead_app_xrd, ".ref", NULL);
  ...
  /* We don't GC per-app directories */
  g_assert_no_errno (stat (alive_app_dir, &stat_buf));
  g_assert_no_errno (stat (dead_app_dir, &stat_buf));

  /* We don't GC XDG_RUNTIME_DIR */
  g_assert_no_errno (stat (alive_app_xrd, &stat_buf));
  g_assert_no_errno (stat (dead_app_xrd, &stat_buf));

  /* We do GC /tmp if all instances have exited */
...
test_claim_per_app_temp_directory (void)
{
   ...
   * that we own, if it doesn't contain the lock file that indicates that
  ...
  g_assert_true (g_str_has_prefix (error->message,
                                   "opening lock file /tmp/flatpak-com.example.App-"));
  ...
  /* Create the lock file (of course in real life this would have happened
   * much sooner) */
  lock_path = g_build_filename (dir_in_tmp, ".flatpak-tmpdir", NULL);
  g_file_set_contents (lock_path, "", 0, &error);
  g_assert_no_error (error);

  /* A conflicting lock doesn't work */
    {
      g_autoptr(GBytes) bytes = NULL;
      g_autofree char *hold_lock = g_test_build_filename (G_TEST_BUILT, "hold-lock", NULL);
      g_autofree char *lock = g_build_filename (dir_in_tmp, ".flatpak-tmpdir", NULL);
      const char *hold_lock_argv[] =
      {
        "<BUILT>/hold-lock",
        "--lock-file",
      ...
      g_assert_nonnull (error);
      g_assert_true (g_str_has_prefix (error->message,
                                       "cannot lock file /tmp/flatpak-com.example.App-"));
      g_assert_nonnull (strstr (error->message, "/.flatpak-tmpdir:"));
      g_assert_null (result);
      g_assert_cmpint (lock_fd, ==, -1);
      g_assert_false (ok);
  ...
  glnx_close_fd (&lock_fd);
  g_assert_no_errno (unlink (lock_path));
}
